...
pub fn path_following_system() -> impl legion::systems::Runnable {
    legion::SystemBuilder::new("PathFollowingSystem")
        ...
        .build(move |_command_buffer, world, resources, query| {
            ...
            for (entity, pos, _agent_type, hunger, _energy, maybe_target, maybe_path, agent_state) in query.iter_mut(world) {
            ...
            for (entity, pos, agent_type, hunger, _energy, mut maybe_target, mut maybe_path, agent_state) in query.iter_mut(world) {
                if *agent_state == AgentState::Idle || *agent_state == AgentState::Arrived {
                    ...
                    if let Some((action, ax, ay)) = possible_actions.choose(&mut rng) {
                            ...
                            "seek_food" => {
                                if let Some(ref mut target) = maybe_target {
                                    ...
                                    if let Some(ref mut path) = maybe_path {
                                        if let Some(astar_path) = a_star_path(&resources.0, agent_type, (pos.x as i32, pos.y as i32), (*ax as i32, *ay as i32), 120) {
                        ...
                        // Wander
                        let (rx, ry) = crate::navigation::random_passable_target(&resources.0, agent_type, &mut rng, Some((pos.x, pos.y)));
                        if let Some(ref mut target) = maybe_target {
                            target.x = rx;
                            target.y = ry;
                            if !log_config.quiet {
                            ...
                            if let Some(ref mut path) = maybe_path {
                                if let Some(astar_path) = a_star_path(&resources.0, agent_type, (pos.x as i32, pos.y as i32), (rx as i32, ry as i32), 120) {
                                    path.waypoints = astar_path.into_iter().collect();
                                    if !log_config.quiet {
                                        resources.2.push(format!("[PATHFIND] Path assigned: {} waypoints", path.waypoints.len()));
                                    }
